"""
TEST 3 — Analyze a PDF loan application against the rules extracted in Test 1.

Reads rules.json written by test_1_extract_rules.py.

Usage:
    python test_3_analyze_pdf.py
    python test_3_analyze_pdf.py --cached   # reuse saved analysis_pdf.json, regenerate HTML only
"""
...
analysis_path   = OUT_DIR / "analysis_pdf.json"
report_path     = OUT_DIR / "report_pdf.html"
...
print(f"Doc    : {loan_pdf_name}  ({len(doc_text):,} chars)  mode={'CACHED' if USE_CACHE else 'LIVE AI'}\n")
...
if USE_CACHE and analysis_path.exists():
    ...
    if USE_CACHE:
        print("WARNING: no analysis_pdf.json found — running AI anyway")
