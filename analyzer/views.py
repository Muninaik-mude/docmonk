...
class ClauseAnalyzerView(APIView):
    ...
    4. Analyze each clause via Groq AI + annotate original PDF
    5. Generate report (PDF / Markdown / both) from analysis_summary
    6. Upload report(s) to S3/R2
    7. Return analysis_summary + report download URL(s)
    """
    ...
    def post(self, request):
        ...
        # Step 3: Analyze each clause via Groq + build annotation list
        analysis_summary = []
        annotations = []
        ...
        # Step 4: Generate report(s) from analysis_summary
        run_id = uuid.uuid4()
        response_data = {
            "status": "success",
        ...
        try:
            if report_format in ("pdf", "both"):
                pdf_report_bytes = report_service.generate_pdf_report(analysis_summary)
            ...
            if report_format in ("markdown", "both"):
                md_content = report_service.generate_markdown_report(analysis_summary)
