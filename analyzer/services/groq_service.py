...
{{
    ...
    "ai_recommendation": "If VIOLATION or PARTIALLY_SATISFIED: rewrite the FULL sentence(s) from relevant_text — keep the exact same sentence structure, subject, and legal language, only substituting the non-compliant values with the correct values from the required clause. The rewritten sentence must be approximately the same length and completeness as the original relevant_text — never a short fragment or a bare value. Copy the EXACT leading prefix from relevant_text character-for-character: (a) if relevant_text starts with a number prefix (e.g. '2. Monthly Rent'), begin with that exact prefix; (b) if relevant_text starts with a bullet+number (e.g. '• 2. Monthly Rent'), keep both; (c) if no prefix, add none. If NOT_FOUND: write a complete, standalone legal clause sentence for this obligation — do NOT add any numbering prefix. If MATCH: null",
